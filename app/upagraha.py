...
def compute_mandi_longitude(dob: date, tob: time, utc_offset: float, lat: float, lon: float) -> float:
    """Mandi (Gulika): the ascendant rising at the start of Saturn's 1/8 segment
    of the day (sunrise-sunset) or night (sunset-sunrise) containing the birth,
    per the weekday tables in app.constants. See app/upagraha.py commit message
    / conversation history for sources — conventions vary between traditions."""
    geopos = (lon, lat, 0)
    ...
    _, ascmc = swe.houses_ex(jd_segment_start, lat, lon, b"W", sid_flags)
    return ascmc[0] % 360
